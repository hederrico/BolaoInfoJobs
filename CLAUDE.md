...
Icons are inline SVG (`ICONS`/`icon()`), not emoji. The scenario grid is `table-layout:fixed` so it fits mobile width; outcome cells show only a flag (winner) or `=` (draw).
