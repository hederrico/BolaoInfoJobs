...
- **"O que está em jogo"** (`stakesFor()`/`buildStakes()`) — in live/upcoming group cards: for each of the two teams, the possible finishing positions if THIS game ends win/draw/loss (marginalizes the other remaining games via full scoreline enumeration). Cached in `stkCache` by `groupSig`. Rendered as **one compact line per team** (flag + name on the left, three right-aligned `V`/`E`/`P` chips with positions, color-coded ok/mid/out via `posClass`).
- **Share** — two canvas builders, both **no external lib, no CORS** (text-only, no flag images): `makeShareCanvas()` draws the branded standings PNG (statusbar button → `shareRanking()`); `makeMatchCanvas(g)` draws a single-match PNG (scoreline + each player's prediction & points). Both go through `openShareModal(canvas, filename, title)` — an **in-app preview modal** (`#shmodal`, `max-height:90vh`, scrollable image) so the result is always visible (the old direct-`navigator.share` path hid the image behind an OS sheet). The modal offers **Baixar imagem** (`doDownloadBlob`) always and **Compartilhar** (`doShareBlob` → `navigator.share({files})`) only when `navigator.canShare` supports files. Per-panel share buttons (`.cardshare`) sit in each live card header and in each history card body; the global ranking share stays in the statusbar.
